...
acpi_ndev_init(struct led_data *phytnet_led)
{
	...
	err = device_property_read_string(&phytnet_led->pdev->dev,
					  NET_DEV_PROPNAME, &ndev_acpi_path);
	if (err) {
	...
	if (ACPI_FAILURE(status)) {
		dev_err(&phytnet_led->pdev->dev, "Failed to get acpi handler on path: %s\n",
			ndev_acpi_path);
		return -ENODEV;
	}

	err = acpi_bus_get_device(net_handler, &adev);
	if (err) {
...
gpio_init(struct led_data *phytnet_led)
{
	...
	phytnet_led->link = gpiod_get_index(&phytnet_led->pdev->dev, LED_OF_NAME,
					    LINK_OFFSET, GPIOD_OUT_HIGH);
	if (IS_ERR(phytnet_led->link)) {
		dev_err(&phytnet_led->pdev->dev, "Failed to get link led gpio, err code: %ld\n",
	...
	if (err) {
		dev_err(&phytnet_led->pdev->dev, "Failed to set linkled dir, err code: %ld\n",
			PTR_ERR(phytnet_led->link));
		return err;
	}

	phytnet_led->act = gpiod_get_index(&phytnet_led->pdev->dev, LED_OF_NAME,
					   ACT_OFFSET, GPIOD_OUT_HIGH);
	if (IS_ERR(phytnet_led->act)) {
		dev_err(&phytnet_led->pdev->dev, "Failed to get actled gpio, err code:%d\n", err);
		return PTR_ERR(phytnet_led->act);
	}

	err = gpiod_direction_output(phytnet_led->act, LED_OFF);
	if (err) {
		dev_err(&phytnet_led->pdev->dev, "Failed to set act led gpio dir, err:%d\n", err);
		return err;
	}
...
{
	struct led_data *phytnet_led;

	phytnet_led = devm_kzalloc(&pdev->dev, sizeof(struct led_data), GFP_KERNEL);
	if (!phytnet_led)
		return -ENOMEM;
