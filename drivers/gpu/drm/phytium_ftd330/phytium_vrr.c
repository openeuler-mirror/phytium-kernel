...
void phytium_change_fps(struct phytium_dp_device *phytium_dp, bool enable, int freq)
{
	...
	struct dc_hw_display_mode old_mode;
	struct dc_hw_display_mode vrr_mode;
	unsigned int num, den, old_refresh = 0;
	...
	old_refresh = DIV_ROUND_CLOSEST_ULL(mul_u32_u32(num, 1000), den);
	if (!enable) {
#if VRR_DEBUG
		pr_info("disable vrr\n");
#endif
	...
	if (freq > old_refresh) {
		DRM_INFO("vfreq %d Hz is higher than current mode freq %d\n",
			freq, old_refresh);
		return;
	}
