...
void phytium_psr_init_dpcd(struct phytium_dp_device *phytium_dp)
{
	...
	if (phytium_dp->psr_dpcd[0] == 1)
		phytium_dp->psr.sink_no_train_on_exit = val & DP_PSR_NO_TRAIN_ON_EXIT;
	else
		phytium_dp->psr.sink_no_train_on_exit = true;
	...
	phytium_dp->psr.disabled_psr_time = ktime_set(0, 0);
...
static int phytium_psr_activate(struct phytium_dp_device *phytium_dp)
{
	struct ftd330_drm_private *priv = phytium_dp->dev->dev_private;
	int port = phytium_dp->port;
	uint32_t group_offset = priv->dp_reg_base[port];
	...
	u32 dc_offset;
	bool sink_needs_recovery = false;
	...
	usleep_range(phytium_dp->psr.psr_setup_time_us, phytium_dp->psr.psr_setup_time_us + 100);
	...
	if (ret) {
		DRM_INFO("Timeout for sink to transition from inactive to display from RFB\n"
					"Sink state is %ld\n", sink_status);
		...
		10000, 100000);
		if (ret) {
			DRM_ERROR("Timeout for Source to transition from active to inactive\n"
					"Source state is %ld\n", source_status);
			}
		...
		phytium_dp->psr.sink_status_error_count++;
		if (phytium_dp->psr.sink_status_error_count >= 3)
			phytium_dp->psr.sink_not_reliable = true;
		
		sink_status = phytium_dp_get_sink_psr_status(phytium_dp);
		if (sink_status != DP_PSR_SINK_INACTIVE)
			sink_needs_recovery = true;
		if (sink_needs_recovery) {
			DRM_INFO("Recover panel state upon power cycling\n");
			phytium_dp->psr.sink_not_reliable = true;
...
void phytium_psr_enable(struct phytium_dp_device *phytium_dp)
{
	...
	if (phytium_dp->psr.sink_not_reliable) {
		DRM_DEBUG_KMS("Sink PSR is not reliable, disable psr\n");
		return;
	}
	phytium_wait_time_cycle(ktime_get_boottime(),
		phytium_dp->psr.disabled_psr_time,
		1000 / phytium_dp->freq);

	phytium_psr_enable_sink(phytium_dp);
	phytium_psr_enable_source(phytium_dp);

	if (phytium_psr_activate(phytium_dp)) {
		DRM_INFO("PSR active failed\n");
		return;
	}
#if PSR_DEBUG
	...
	phytium_dp->psr.enabled_psr_time = ktime_get_boottime();
}
...
static void phytium_psr_disable_source(struct phytium_dp_device *phytium_dp)
{
	...
	ret = 0;
	if (!phytium_dp->psr.sink_no_train_on_exit) {
		if (phytium_dp->fast_train_support)
			ret = phytium_dp_fast_link_train(phytium_dp);
		if (!ret) {
			phytium_dp_start_link_train(phytium_dp, false);
		}
		mdelay(2);
	} else {
...
void phytium_psr_disable(struct phytium_dp_device *phytium_dp)
{
	...
	phytium_wait_time_cycle(ktime_get_boottime(),
		phytium_dp->psr.enabled_psr_time,
		1000 / phytium_dp->freq);
	...
	msleep(phytium_dp->psr.sink_sync_latency * (1000 / phytium_dp->freq));
	ret = readx_poll_timeout(phytium_dp_get_sink_psr_status, phytium_dp, sink_status,
		sink_status >= 0 && (
		(sink_status == DP_PSR_SINK_INACTIVE) ||
		(sink_status == DP_PSR_SINK_ACTIVE_RESYNC)),
		1500, 300000);
	if (ret) {
		DRM_INFO("Timeout for sink to transition to inactive\n"
				"Sink state is %ld\n", sink_status);
		ret = drm_dp_dpcd_readb(&phytium_dp->aux, DP_LAST_RECEIVED_PSR_SDP, &val);
		DRM_INFO("LAST RECEIVED PSR SDP: 0X%02x in %s\n", val, __func__);
		phytium_dp_start_link_train(phytium_dp, true);
		phytium_dp->psr.sink_status_error_count++;
		if (phytium_dp->psr.sink_status_error_count >= 3)
			phytium_dp->psr.sink_not_reliable = true;
	...
	phytium_dp->psr.disabled_psr_time = ktime_get_boottime();
}
